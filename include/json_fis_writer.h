...
public:
  JsonFISWriter(const SingletonFIS &fis, size_t n_true_labels,
                ...
                const vector<vector<double>> &cons_range, string &json_output)
      : fis{fis}, n_true_labels{n_true_labels},
        n_labels_per_cons{n_labels_per_cons},
        n_classes_per_cons{n_classes_per_cons}, vars_range{vars_range},
        cons_range{cons_range}, json_output{json_output} {};
  ...
  virtual void write() {
    ...
    j_out["n_classes_per_cons"] = n_classes_per_cons;
    ...
    for (auto &rule : fis.rules) {
      ...
      for (auto &ant : rule.ants) {
        ...
        auto lv_json =
            scale_back_var(ant.second.lv.in_values, vars_range[ant.first]);
  ...
  vector<double> scale_back_var(const vector<double> &in_values,
    ...
    transform(in_values.begin(), in_values.end(), in_values_scaled.begin(),
              [&var_range](double in_value) {
                return in_value * (var_range[1] - var_range[0]) + var_range[0];
              });
